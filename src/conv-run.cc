...
enum qnnp_status qnnpackConv(
  ...
  if (conv_p.ukernel_type == qnnp_ukernel_type_xzp_gemm) {
    requantization_params = qnnp_compute_requantization_params(
        convolution_scale,
        output_zero_point,
        conv_p.output_min,
        conv_p.output_max);
  } else {
    conv_quantization_params = qnnp_compute_conv_quantization_params(
        input_zero_point,
  ...
  convolution =
      static_cast<qnnp_operator_t>(calloc(1, sizeof(struct qnnp_operator)));
  if (convolution == nullptr) {
    qnnp_log_error(
        "failed to allocate %zu bytes for qnnp_operator structure",
        sizeof(struct qnnp_operator));
    return qnnp_status_out_of_memory;
  }

  std::unique_ptr<qnnp_operator, QnnpackDeleter> qnnpack_uniq_ptr(convolution);
  ...
  switch (conv_p.ukernel_type) {
    ...
    default: {
      qnnp_log_error("Invalid kernel type. QNNPACK convolution run failed.");
      QNNP_UNREACHABLE;
    }
  }
